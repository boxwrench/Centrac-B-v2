...
export function createPumpScene(host:HTMLElement,onSelect:(id:string|null)=>void,onPhase:(phase:number)=>void){
 ...
 let state:ViewState={explode:0,xray:false,shellOpacity:.025,cutaway:false,cutSide:1,running:false,speed:.5,labels:false,selected:null,isolate:false,phase:0,mode:'assembled'};let explosion=0;let theta=0;let frame=0;let last=performance.now();let phaseTick=0;let active=true;
 ...
 function animate(now:number){if(!active)return;frame=requestAnimationFrame(animate);const dt=Math.min((now-last)/1000,.05);last=now;if(state.running)theta+=dt*Math.PI*2*state.speed;explosion=THREE.MathUtils.damp(explosion,state.explode,6,dt);for(const[id,g]of Object.entries(groups))g.position.copy(base.get(g)!).addScaledVector(offsets[id],explosion);

...
import { Box, Layers, Activity, Scissors, Search, RotateCcw, Maximize, Plus, Minus, Play, Pause, ChevronRight, ArrowUpRight, Focus, Eye, Tag, Download, BookOpen, X, MousePointer2, Check, Copy } from 'lucide-react';
  ...
  return <div className="cb-model">
    <main className="explorer">
      ...
      <section className={`viewport ${xray || cutaway ? 'inspecting' : ''}`} aria-label="3D pump explorer"><div ref={host} className="canvas-host" />{!ready && !error && <div className="loading"><Box size={28} /><span>Preparing the pump assembly…</span></div>}{error && <div className="loading error">{error}</div>}
        ...
        {part && <div className="inspection"><div className="inspection-top"><span className="eyebrow">{part.section.toUpperCase()} / ITEM {part.item}</span><button aria-label="Close part details" onClick={() => { setSelected(null); setIsolate(false) }}><X size={15} /></button></div><h3>{part.name}</h3><div className="pn"><code>{part.pn}</code><button aria-label="Copy part number" onClick={async () => { try { await navigator.clipboard.writeText(part.pn); setCopied(true); setTimeout(() => setCopied(false), 1500) } catch { setCopied(false) } }}>{copied ? <Check size={14} /> : <Copy size={14} />}</button></div><p>{part.description}</p><div className="part-meta"><span>QUANTITY<b>{part.qty}</b></span><span>REFERENCE<b>Fig. {part.figure} · p. {part.page}</b></span></div><div className="inspection-actions"><button onClick={() => selected && engine.current?.focus(selected)}><Focus size={14} />Focus</button><button className={isolate ? 'enabled' : ''} onClick={() => { setIsolate(v => !v); setTimeout(() => selected && engine.current?.focus(selected), 30) }}><Eye size={14} />{isolate ? 'Show all' : 'Isolate'}</button><a href={`${PDF}#page=${part.page + (part.figure === 8 ? 6 : 5)}`} target="_blank" rel="noreferrer" title="Open original parts table"><BookOpen size={16} /></a></div></div>}
      ...
      {sources && <div className="modal-backdrop" onClick={() => setSources(false)}><section className="reference-modal" role="dialog" aria-modal="true" aria-label="Reference library" onClick={e => e.stopPropagation()}><div className="modal-heading"><BookOpen size={24} /><button aria-label="Close reference library" onClick={() => setSources(false)}><X /></button></div><span className="eyebrow">SOURCE MATERIAL</span><h2>Grounded in the manual.</h2><p>The model reconstructs the simplex drive and a 106 mm metallic HPD liquid end from the supplied Centrac B IOM. Part numbers are transcribed for the selected configuration.</p><a className="source-link" href={PDF} target="_blank" rel="noreferrer"><BookOpen /><span><b>Centrac B · Installation & operation manual</b><small>56 PDF pages · parts tables on printed pages 21–32</small></span><ArrowUpRight /></a><a className="source-link" href={`${PDF}#page=26`} target="_blank" rel="noreferrer"><Layers /><span><b>Figure 8 · Simplex drive</b><small>Drawing 102-2925-0001 · printed page 20</small></span><ArrowUpRight /></a><a className="source-link" href={`${PDF}#page=40`} target="_blank" rel="noreferrer"><Layers /><span><b>Figure 10 · Metallic liquid end</b><small>Drawing 102-2925-0006 · PDF page 40</small></span><ArrowUpRight /></a><a className="source-link" href="https://www.miltonroy.com/en-nam/metering-pumps/centrac-series/b-model/" target="_blank" rel="noreferrer"><Box /><span><b>Milton Roy · Official product page</b><small>Manufacturer overview and specifications</small></span><ArrowUpRight /></a><div className="reference-note"><b>About this reconstruction</b><p>Geometry, clearances, gear tooth forms, motor details and motion timing are illustrative. Part numbers vary by motor, plunger, material and pressure. Confirm the installed model and serial number before ordering. This is an independent viewer, not a manufacturer CAD model or service procedure.</p></div></section></div>}
    </main>
  </div>;
}

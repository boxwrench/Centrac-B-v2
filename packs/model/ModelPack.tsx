...
import { Box, Layers, Activity, Scissors, Search, RotateCcw, Maximize, Plus, Minus, Play, Pause, ChevronRight, ArrowUpRight, Focus, Eye, EyeOff, Tag, Download, BookOpen, X, MousePointer2, Check, Copy, Expand } from 'lucide-react';
...
/**
 * The Centrac B workspace: 3D explorer (`view="model"`) and O&M reader (`view="manual"`).
 * App keeps this mounted once visited so the WebGL scene survives tab switches;
 * rendering pauses whenever the explorer is not on screen.
 */
export default function ModelPack({ view, visible }: { view: 'model' | 'manual'; visible: boolean }) {
  const nav = useNav();
  ...
  const [manualSeen, setManualSeen] = useState(view === 'manual');
  useEffect(() => { if (view === 'manual') setManualSeen(true) }, [view]);
  const onScreen = visible && view === 'model';
  ...
  useEffect(() => {
    ...
    import('./pumpScene').then(({ createPumpScene }) => {
      ...
      try {
        ...
        engine.current.setActive(onScreen);
        setReady(true);
  ...
  useEffect(() => { engine.current?.setActive(onScreen) }, [onScreen, ready]);
  useEffect(() => { if (nav.partRequest && ready) openPart(nav.partRequest.id) }, [nav.partRequest, ready]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  function openManual(page = 1) { setSources(false); nav.openManual(page) }
  function openPart(id: string) { setSelected(id); setIsolate(false); changeMode('cutaway'); setTimeout(() => engine.current?.focus(id), 120) }
  ...
  return <div className="cb-model">
    <main className="explorer">
      <div className="model-panel" hidden={view !== 'model'}><aside className="sidebar"><div className="sidebar-heading"><span className="eyebrow">MODEL CONFIGURATION</span><h1>Inside the Centrac B</h1><p>Simplex · 1″ plunger · 316SS liquid end</p><span className="configuration-tag">1 HP DRIVE <span> / </span> SINGLE-BALL CHECKS</span><button className="manual-button sidebar-sources" onClick={() => setSources(true)}><BookOpen /> Sources & references <ArrowUpRight size={13} /></button></div><div className="parts-heading"><h2>Components <span>{parts.length}</span></h2><button title="Export illustrated parts as CSV" aria-label="Export illustrated parts as CSV" onClick={exportParts}><Download size={16} /></button></div><div className="search"><Search size={15} /><input aria-label="Search parts by name, item or number" placeholder="Find a part or part number…" value={query} onChange={e => setQuery(e.target.value)} /></div><div className="filter-tabs">{['All', 'Drive', 'Liquid end'].map(s => <button key={s} className={s === section ? 'active' : ''} onClick={() => setSection(s)}>{s}</button>)}</div><div className="part-list">{filtered.length === 0 ? <p className="empty">No matching components.<br />Try a name, item, or part number.</p> : filtered.map(p => <button key={p.id} className={`part-row ${selected === p.id ? 'selected' : ''}`} onClick={() => { setSelected(p.id); setIsolate(false) }}><span className={`part-item ${p.section === 'Liquid end' ? 'liquid-item' : ''}`}>{p.item}</span><span className="part-text"><b>{p.name}</b><code>{p.pn}</code></span><ChevronRight size={13} /></button>)}</div><div className="sidebar-footer"><span className="tiny-dot" />Part numbers from the supplied IOM<span>Illustrated selection · not a full bill of materials</span></div></aside>
        <section className={`viewport ${xray || cutaway ? 'inspecting' : ''}${(xray || cutaway) && !isolate ? ' stage-dim' : ''}${uiHidden ? ' ui-hidden' : ''}`} aria-label="3D pump explorer"><div ref={host} className="canvas-host" />{!ready && !error && <div className="loading"><Box size={28} /><span>Preparing the pump assembly…</span></div>}{error && <div className="loading error">{error}</div>}
      ...
      {manualSeen && <div className="document-panel" hidden={view !== 'manual'}><ManualReader page={nav.manualPage} onPage={nav.setManualPage} /></div>}

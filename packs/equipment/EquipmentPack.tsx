      ...
      <section className="bg-surface p-6 rounded-2xl shadow-sm border border-line">
        ...
        <form onSubmit={addEquipment} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
          <div className="space-y-1">
            <label className="text-xs font-semibold text-slate-600 uppercase">Tag *</label>
          ...
          <div className="space-y-1">
            ...
            <select value={type} onChange={(e) => setType(e.target.value as EquipmentType)}
              className="w-full p-2 border border-slate-300 rounded-lg outline-none bg-surface">
          ...
          <div className="space-y-1">
            <label className="text-xs font-semibold text-slate-600 uppercase">Model</label>
            ...
            {equipment.map((eq: Equipment) => (
              <button key={eq.id} onClick={() => setActiveAssetId(eq.id)}
                className={`w-full text-left p-3 rounded-xl border transition-all ${
                  activeAssetId === eq.id ? 'border-orange-500 bg-orange-50 ring-1 ring-orange-500' : 'border-line hover:bg-raised'
                }`}>
